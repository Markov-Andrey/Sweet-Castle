...
@section('content')
    ...
    'So come and join us in our fairytale land and indulge in the best yummy in the whole world!'
        ...
        <div class="flex flex-auto justify-center px-[15%] items-center">
            @if($loop->odd)
                <img class="h-56 rounded-full shadow-2xl" src="images/{{ $image }}" alt="image{{ $loop->iteration }}">
                <p class="sweet-font text-2xl m-10 p-5 bg-white bg-opacity-75 rounded-lg shadow-2xl">
                    {{ $str }}
                </p>
            @else
                <p class="sweet-font text-2xl m-10 p-5 bg-white bg-opacity-75 rounded-lg shadow-2xl">
